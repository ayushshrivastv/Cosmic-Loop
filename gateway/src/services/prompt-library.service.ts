...
// Define the structure for a prompt parameter
...
Your goal is to provide the most helpful response possible based on the user's query and context.`,
    ...
    logger.info(`Initialized prompt library with ${this.templates.size} templates`);
  }
  ...
  addTemplate(template: PromptTemplate): void {
    ...
    logger.debug(`Added template: ${template.id}`, { templateName: template.name });
  }
   ...
   */
  useTemplate(templateId: string, parameters: Record<string, any>): { systemPrompt: string; userPrompt: string } {
   ...
   */
  detectTemplate(query: string): string {
   ...
   */
  processQuery(query: string): { systemPrompt: string; userPrompt: string } {
    // Detect the appropriate template
    const templateId = this.detectTemplate(query);
    logger.debug(`Detected template for query: ${templateId}`, { query: query.substring(0, 100) });
    ...
    logger.debug(`Using template ${templateId} with parameters:`, parameters);
