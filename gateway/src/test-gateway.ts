...
async function testUseTemplate(templateId: string) {
  try {
    console.log(`Testing use template endpoint with template ID: ${templateId}...`);
    
    const response = await axios.post(
      `${GATEWAY_URL}/templates/${templateId}/use`,
      {
...
async function testHealth() {
  try {
    ...
    const response = await axios.get(`${GATEWAY_URL.replace('/api/v1', '')}/health`);
