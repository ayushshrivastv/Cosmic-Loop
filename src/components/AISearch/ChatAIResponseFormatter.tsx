...
interface ChatAIResponseFormatterProps {
  content: string;
  metadata?: {
    latency?: {
      total: number;
...
/**
 * ChatAIResponseFormatter component
 * 
 * A version of AIResponseFormatter adapted for the chat interface
 */
export function ChatAIResponseFormatter({ 
  content, 
  metadata, 
  className = '' 
}: ChatAIResponseFormatterProps) {
  ...
  const customComponents = {
    ...
    code: (props: React.HTMLProps<HTMLElement> & { inline?: boolean; className?: string }) => {
      ...
      return !inline ? (
        <div className="bg-gray-800 rounded-md overflow-x-auto my-2">
          <code 
    ...
    <div className={`${className}`}>
      ...
      {sourcesSection && (
        <div className="mt-4 pt-3 border-t border-gray-700">
          <h4 className="text-sm font-medium mb-2 text-gray-300">Sources:</h4>
      ...
      {metadata && (
        <div className="mt-3 pt-3 border-t border-gray-700 flex flex-wrap gap-2">
          {metadata.latency && (
            <Badge variant="secondary" className="flex items-center gap-1 text-xs bg-gray-200 text-gray-800 border-gray-300">
